...
export async function GET(request: Request) {
    ...
    let res = await axios.post("https://api-preprod.phonepe.com/apis/pg-sandbox/pg/v1/pay", {
        request: data.toString("base64")
    }, {
        headers: {
            "X-VERIFY": SHA256(`${data.toString("base64")}${api_endpoint}${salt_key}`).toString() + "###" + salt_index
        }
    })
    return new Response(
